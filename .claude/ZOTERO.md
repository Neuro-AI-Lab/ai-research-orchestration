...
needed on this machine). One tool, two surfaces: a CLI agents call via Bash, and an MCP server
(`zotero` in `.mcp.json`, loads at session start).
...
2. Fill `.claude/settings.local.json` (gitignored):
   `"ZOTERO_API_KEY": "…", "ZOTERO_USER_ID": "…"` — or `ZOTERO_GROUP_ID` for a shared group
   library instead.
3. Verify: `python3 .claude/scripts/zotero_mcp.py collections` — should list your collections.
...
```bash
python3 .claude/scripts/zotero_mcp.py search "eeg emotion" --limit 10 [--tag HYP-001]
python3 .claude/scripts/zotero_mcp.py item ABCD1234        # metadata + attachment/note keys
python3 .claude/scripts/zotero_mcp.py fulltext EFGH5678    # indexed text of a PDF attachment
python3 .claude/scripts/zotero_mcp.py bibtex ABCD1234,WXYZ9876   # BibTeX for the paper's .bib
python3 .claude/scripts/zotero_mcp.py collections
python3 .claude/scripts/zotero_mcp.py add --title "…" --doi 10.x/y --venue "…" --date 2025 --tag HYP-002
```
...
| Symptom | Meaning | Fix |
|:--|:--|:--|
| "Zotero not configured" | env vars missing | fill `.claude/settings.local.json`, restart session (env loads at start) |
