...
If any root doc or experiments/ status changed after handoff.json was last updated, the first
...
WATCH = ['discussion.md', 'error.md', 'result.md', 'version.md']
...
def main():
    ...
    handoff_mtime = os.path.getmtime(handoff) if os.path.isfile(handoff) else 0.0
    ...
    print(json.dumps({
        'decision': 'block',
        'reason': (
            'Session-close recording is stale: root docs or experiment status changed after '
            ...
            'in discussion.md (dispatch the orchestrator for that — root docs are agent-owned); '
