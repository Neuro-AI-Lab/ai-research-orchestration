...
1. Get a git token: Overleaf → Account Settings → **Git Integration** → Generate token
   ...
   include it).
2. Copy `.codex/settings.local.json.example` to `.codex/settings.local.json` and paste your
   token into `OVERLEAF_GIT_TOKEN`. That file is gitignored (as are `docs/paper-codex*/` clones) —
...
- Rotate the token any time at Overleaf → Account Settings → Git Integration; then update
  `.codex/settings.local.json` — nothing else references it (clones embed it in their remote URL,
  so re-clone or `git remote set-url` after a rotation).
- Never put the token in `settings.json`, agent specs, or Codex research docs (those are checked in).
...
2. Clone it (pick a name for multi-project setups; the default dir is `docs/paper-codex`):

   ```bash
   .codex/scripts/overleaf_sync.sh clone <project-id> docs/paper-codex-<short-name>
   ```

3. Verify: `.codex/scripts/overleaf_sync.sh status docs/paper-codex-<short-name>` — remote shows
...
| Action | Command |
|:--|:--|
| Get latest (incl. your web edits) | `.codex/scripts/overleaf_sync.sh pull docs/paper-codex-<name>` |
| Push agent edits | `.codex/scripts/overleaf_sync.sh push docs/paper-codex-<name> "writer: results (EXP-003)"` |
| Check state | `.codex/scripts/overleaf_sync.sh status docs/paper-codex-<name>` |
...
| `Authentication failed` | token revoked/rotated | new token → update `.codex/settings.local.json`; re-clone or `git remote set-url` |
