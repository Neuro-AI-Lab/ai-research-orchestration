...
Configuration (put in .codex/settings.local.json "env"; see .codex/docs/integrations/ZOTERO.md):
...
CLI usage (agents call this via Bash):
  ...
  zotero_mcp.py serve                    # MCP stdio server (registered in .codex/config.toml)
"""
import json
import os
import sys
...
def config():
    ...
    if local:
        ...
        raise SystemExit(
            ...
            '.codex/settings.local.json, or ZOTERO_LOCAL=1 with the desktop app running. '
            'Setup guide: .codex/docs/integrations/ZOTERO.md')
